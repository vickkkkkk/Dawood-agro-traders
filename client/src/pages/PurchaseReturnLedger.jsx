...
import { 
  ...
  Tag, RefreshCw, FileText, CheckCircle, Info, Truck
} from 'lucide-react';
...
const PurchaseReturnLedger = () => {
  ...
  // Fetch Return Records for Purchases
  const { data: returnsData, isLoading, refetch } = useQuery({
    queryKey: ['purchase-returns', search, startDate, endDate],
    queryFn: () => getReturns({
      type: 'PURCHASE',
      search,
      startDate,
  ...
  const handlePrint = () => {
    const printContent = document.getElementById('pur-return-receipt-print').innerHTML;
  ...
  return (
    <div className="app-container animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-gradient-to-br from-red-600/20 to-orange-850/20 border-red-500/20">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm font-medium text-slate-400 uppercase tracking-wider">Total Purchase Returns</p>
              <p className="text-3xl font-extrabold text-red-400 mt-1">{formatCurrency(totalAmount)}</p>
            </div>
            <div className="w-12 h-12 rounded-xl bg-red-500/10 flex items-center justify-center border border-red-500/20 shadow-lg">
              <RotateCcw size={24} className="text-red-400 animate-spin-slow" />
            </div>
          </div>
        </Card>

        <Card className="bg-gradient-to-br from-indigo-600/20 to-blue-800/20 border-indigo-500/20">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm font-medium text-slate-400 uppercase tracking-wider">Total Return GRNs</p>
              <p className="text-3xl font-extrabold text-blue-400 mt-1">{totalCount} Returns</p>
            </div>
            <div className="w-12 h-12 rounded-xl bg-blue-500/10 flex items-center justify-center border border-blue-500/20 shadow-lg">
              <FileText size={24} className="text-blue-400" />
            </div>
          </div>
        </Card>

        <Card className="bg-gradient-to-br from-amber-600/20 to-yellow-800/20 border-amber-500/20">
          <div className="flex items-start justify-between">
            <div>
      ...
      <Card compact>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
          <div>
            <SearchBar
              id="pur-return-search"
              placeholder="Search return # or GRN #..."
              value={search}
      ...
      <Card padding={false}>
        <div className="overflow-x-auto" style={{ maxHeight: 'calc(100vh - 340px)' }}>
          <Table
            ...
            loading={isLoading}
            headers={['Return ID', 'Return Date', 'Ref GRN', 'Supplier', 'Reason', 'Refund Method', 'Net Value', 'Processed By', 'Actions']}
            ...
            {returnRecords.length > 0 ? (
              returnRecords.map((record) => (
                <tr key={record.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-slate-300 font-sans">
                    {record.supplier?.name} {record.supplier?.company && `(${record.supplier.company})`}
                  </td>
              ...
              <tr>
                <td colSpan="9" className="text-center py-8 text-slate-500 font-bold">
                  No purchase returns matched your search criteria.
                </td>
              </tr>
            )}
          </Table>
      ...
      {showViewModal && selectedReturn && (
        <Modal
          id="pur-return-detail-modal"
          title={`Purchase Return Detail: ${selectedReturn.returnNo}`}
          onClose={() => {
        ...
        >
          <div className="space-y-6">
            ...
            <div id="pur-return-receipt-print" className="bg-white text-black p-6 rounded-lg font-sans border border-slate-200">
              <div className="text-center border-b border-gray-300 pb-4 space-y-1">
                ...
                <div className="bg-red-700 text-white py-1 px-3 inline-block rounded text-[10px] font-extrabold uppercase mt-2 tracking-wider">
              ...
              <div className="grid grid-cols-2 gap-y-2 text-xs border-b border-gray-300 py-4 font-medium">
                ...
                <div>
                  <span className="text-gray-500">Original GRN Reference:</span>
                  <p className="font-extrabold text-black font-mono">{selectedReturn.referenceNo}</p>
                </div>
                <div className="text-right">
                  <span className="text-gray-500">Supplier:</span>
                  <p className="font-extrabold text-black">{selectedReturn.supplier?.name} {selectedReturn.supplier?.company && `(${selectedReturn.supplier.company})`}</p>
                </div>
                <div>
                ...
                <div className="text-right">
                  <span className="text-gray-500">Refund/Adjustment:</span>
                  <p className="font-extrabold text-red-700">{selectedReturn.refundMethod}</p>
                </div>
              </div>
              ...
              {/* Returned Items */}
              <div className="pt-2">
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Returned Goods List</p>
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="border-b border-gray-300 font-extrabold text-gray-700 uppercase tracking-wider text-[10px]">
                      ...
                      <th className="py-1.5 text-right">Returned Qty</th>
                      <th className="py-1.5 text-right">Cost Price</th>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button variant="primary" icon={CheckCircle} onClick={handlePrint}>Print Voucher</Button>
            </div>
          </div>
        </Modal>
      )}
...
export default PurchaseReturnLedger;
